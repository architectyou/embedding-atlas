...
def main(host, port, auto_port, duckdb, static):
    ...
    # 정적 파일 경로 설정
    if static is None:
        backend_path = Path(__file__).parent / "packages" / "backend" / "embedding_atlas" / "static"
    ...
    for name, file_path, vector_col in parquet_files:
        if Path(file_path).exists():
            try:
                atlas.add_dataset(name, file_path, vector_col)
            except Exception as e:
